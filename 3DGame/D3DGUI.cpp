...
// Desc: 构造函数
//-----------------------------------------------------------------------------
D3DGUI::D3DGUI(LPDIRECT3DDEVICE9 device, int w, int h)//(界面的宽,高)
{
	m_pFonts = NULL;
	m_pControls = NULL;
...
//-----------------------------------------------------------------------------
// Name： D3DGUI::CreateTextFont( )
// Desc:	 字体创建函数
//-----------------------------------------------------------------------------
bool D3DGUI::CreateTextFont(wchar_t *fontName, int size, int *fontID)
{
...
//-----------------------------------------------------------------------------
// Name： D3DGUI::AddBackground( )
// Desc:	创建保存背景图的缓存函数
//-----------------------------------------------------------------------------
bool D3DGUI::AddBackground(wchar_t *fileName)
{
...
//-----------------------------------------------------------------------------
// Name： D3DGUI::AddStaticText( )
// Desc:	创建静态文本控件
//-----------------------------------------------------------------------------
bool D3DGUI::AddStaticText(int id, wchar_t *text, float x, float y, unsigned long color, int fontID)
{
	...
	wcscpy_s(m_pControls[m_nTotalControlNum].m_text,-1, text);
...
//-----------------------------------------------------------------------------
// Name： D3DGUI::AddButton( )
// Desc:	创建按钮控件的函数
//-----------------------------------------------------------------------------
bool D3DGUI::AddButton(int id, float x, float y, wchar_t *up, wchar_t *over, wchar_t *down)
{
...
//-----------------------------------------------------------------------------
void ProcessGUI(D3DGUI *gui, bool LMBDown, int mouseX, int mouseY, void(*funcPtr)(int id, int state))
{
	if (!gui) return;
