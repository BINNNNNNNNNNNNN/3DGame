...
//--------------------------------------------------------------------------------------
// Name: Terrain::LoadTerrainFromFile()
// Desc: 加载地形高度信息以及纹理
//--------------------------------------------------------------------------------------
BOOL Terrain::LoadTerrainFromFile(wchar_t *pRawFileName, wchar_t *pTextureFile)
{
	// 从文件中读取高度信息
	std::ifstream inFile;
...
//--------------------------------------------------------------------------------------
// Name: Terrain::InitTerrain()
// Desc: 初始化地形的高度, 填充顶点和索引缓存
//--------------------------------------------------------------------------------------
BOOL Terrain::InitTerrain(INT nRows, INT nCols, float fSpace, float fScale, float fUTextureNumbers, float fVTextureNumbers)//fUTextureNumbers,fVTextureNumbers横竖纹理图铺满所用的张数 
{
	...
	float fStartZ = m_fTerrainDepth / 2.0f, fEndZ = -m_fTerrainDepth / 2.0f;	//指定起始点和结束点的Z坐标值
																				//  float fStartX = 0.0f, fEndX =  m_fTerrainWidth;	//指定起始点和结束点的X坐标值
																				//  float fStartZ = m_fTerrainDepth, fEndZ = 0.0f;//指定起始点和结束点的Z坐标值
	float fCoordU = fUTextureNumbers / (float)m_nCellsPerRow;						//指定纹理的横坐标值
	float fCoordV = fVTextureNumbers / (float)m_nCellsPerCol;						//指定纹理的纵坐标值
...
float Terrain::getPositionHeight(D3DXVECTOR3 pos)
{
	return m_vHeightInfo[(100 - (int)pos.x) * m_nCellsPerRow + 100 + (int)pos.z];
}
//-----------------------------------------------------------------------------
// Desc: 析构函数
//-----------------------------------------------------------------------------
